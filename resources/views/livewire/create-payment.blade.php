    ...
    <x-jet-form-section :submit="$action" class="mb-4">
        ...
        <x-slot name="form">
          <div class="form-group col-span-6 sm:col-span-3">
              ...
              <x-btui-select id="order_id" class="mt-1 block w-full form-control shadow-none" name="payment.order_id" :options="$orders" wire:model.defer="payment.order_id" placeholder="Select Order" />
              <x-jet-input-error for="payment.order_id" class="mt-2" />
          </div>

          <div class="form-group col-span-6 sm:col-span-3">
              <x-jet-label for="payment_state_id" value="{{ __('Payment State') }}" />
              <x-btui-select id="payment_state_id" class="mt-1 block w-full form-control shadow-none" name="payment.payment_state_id" :options="$payment_states" wire:model.defer="payment.payment_state_id" placeholder="Select Payment State" />
              <x-jet-input-error for="payment.payment_state_id" class="mt-2" />
          </div>

          <div class="form-group col-span-6 sm:col-span-3">
              <x-jet-label for="payment_method_id" value="{{ __('Payment Method') }}" />
              <x-btui-select id="payment_method_id" class="mt-1 block w-full form-control shadow-none" name="order.payment_method_id" :options="$payment_methods" wire:model.defer="payment.payment_method_id" placeholder="Select Payment Method" />
              <x-jet-input-error for="payment.payment_method_id" class="mt-2" />
          </div>

          <div class="form-group col-span-6 sm:col-span-3">
              <x-jet-label for="installment_id" value="{{ __('Payment Method') }}" />
              <x-btui-select id="installment_id" class="mt-1 block w-full form-control shadow-none" name="payment.installment_id" :options="$installments" wire:model.defer="payment.installment_id" placeholder="Select Payment Method" />
